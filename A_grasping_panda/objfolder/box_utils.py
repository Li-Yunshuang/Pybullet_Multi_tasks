...
def ray_box_intersection_pairwise(box_center,
    ...
    for axis in range(3):
        ...
        for _ in range(2):
            ...
            if exclude_enlarged_t:
                t = torch.maximum(t, torch.tensor(1.0))  # [R,]
    ...
    if len(false_indices) > 0:
        intersection_masks_any[false_indices[:, None]] = torch.zeros(false_indices.size(), dtype=torch.bool)
