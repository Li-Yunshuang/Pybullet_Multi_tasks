...
import numpy as np
from objfolder.TouchNet_utils import *
...
def get_embedder(multires, i=0):
    if i == -1:
        #9 is for x, y, z, theta, phi_x, phi_y, displacement, w, h
        return nn.Identity(), 9
...
class AudioNeRF(nn.Module):
    def __init__(self, D=8, input_ch=5):
        super(AudioNeRF, self).__init__()
        self.model_x = NeRF(D = D, input_ch = input_ch)
        self.model_y = NeRF(D = D, input_ch = input_ch)
        self.model_z = NeRF(D = D, input_ch = input_ch)
