...
class PandaSim(object):
    ...
    def step(self, pos, angle, gripper_w):
        ...
        if self.state == 0:
            ...
            pos = [pos[0], pos[1], 0.1] # 机械手位置
            #orn = self.p.getQuaternionFromEuler([math.pi,0., math.pi / 2])   # 机械手方向
            orn = self.p.getQuaternionFromEuler([-math.pi/2,0., math.pi / 2])   # 机械手方向
            jointPoses = self.calcJointLocation(pos, orn)
            self.setArm(jointPoses)
            return False
    ...
    def localize(self, pos, angle, gripper_w):
        ...
        if self.state == 0:
            ...
            #orn = self.p.getQuaternionFromEuler([math.pi,0., math.pi / 2])   # 机械手方向
            #orn = self.p.getQuaternionFromEuler([-math.pi,0, math.pi / 2])   # 机械手方向 #-math.pi,0, math.pi / 2  #IN world orn
